...
CREATE INDEX IF NOT EXISTS idx_auth_api_contract_endpoint ON public.auth_api_contract (endpoint_path, endpoint_method)
WHERE
    is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_auth_user_api_contract_user ON public.auth_user_api_contract (user_id)
WHERE
    is_active = TRUE;

CREATE UNIQUE INDEX IF NOT EXISTS uq_auth_user_api_contract_active ON public.auth_user_api_contract (user_id, api_contract_id)
WHERE
    is_active = TRUE;
