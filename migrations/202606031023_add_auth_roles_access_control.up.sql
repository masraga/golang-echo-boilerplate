...
CREATE UNIQUE INDEX IF NOT EXISTS uq_auth_roles_active_role_name ON public.auth_roles (role_name)
WHERE
    is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_auth_roles_owner ON public.auth_roles (owner_id)
WHERE
    is_active = TRUE;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_auth_roles_contract_api_active ON public.auth_roles_contract_api (role_id, auth_api_contract_id)
WHERE
    is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_auth_roles_contract_api_role ON public.auth_roles_contract_api (role_id)
WHERE
    is_active = TRUE;
